...
class HelpPaginator(Pages):
    ...
    def prepare_embed(self, entries, page, *, first=False):
        ...
        for entry in entries:
            ...
            self.embed.add_field(name=signature, value=entry.short_doc or "ヘルプはありません。", inline=False)
    ...
    async def show_bot_help(self):
        ...
        entries = (
            ('<引数>', 'これは引数が**必ず必要**という意味です。'),
            ('[引数]', 'これは引数が**オプション**という意味です。'),
            ('[A|B]', 'これは引数が**AもしくはB**ではなければいけないという意味です。'),
            ('[引数...]', 'これは引数が複数行という意味です。\n' \
